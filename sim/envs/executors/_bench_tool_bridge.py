"""Run airline bench tool calls inside tau2-bench's virtualenv.

This script is intentionally executed by ``tau2-bench/.venv/bin/python`` so it
can import tau2 and its pydantic-based data models without polluting the main
PSAgent runtime.
"""
...
from tau2.domains.airline.environment import get_environment, get_tasks  # type: ignore  # noqa: E402
...
def main() -> None:
    ...
    if task is not None and getattr(task, "initial_state", None) is not None:
        env.set_state(task.initial_state, None, [])
