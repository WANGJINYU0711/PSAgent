"""Naive mixed baseline with prefix-local average edge cost ordering.

This policy does not use any shared/unshared structure and does not maintain
whole-path arms. Each prefix locally stores the historical average end-to-end
cost for each selected direct child edge. Selection greedily chooses the legal
child with the smallest stored edge cost, using deterministic prefix ordering to
break ties. There is no epsilon exploration, no theta/EXP3 update, and no
shared delta propagation.
"""
...
class NaiveMixedAveragePolicy(BasePolicy):
    """Prefix-local naive cost-ordering baseline with average edge costs."""

    def __init__(self, seed: int = 0) -> None:
        super().__init__(seed=seed, protocol_mode="actual_leaf")
        self.paths: list[list[str]] = []
        self.edge_cost_sums: dict[tuple[tuple[str, ...], tuple[str, ...]], float] = {}
        self.edge_cost_counts: dict[tuple[tuple[str, ...], tuple[str, ...]], int] = {}

    @property
    def name(self) -> str:
        return "naive_mixed_avg"
    ...
    def _edge_cost(
        ...
        count = int(self.edge_cost_counts.get(edge_key, 0))
        if count <= 0:
            return 0.0
        return float(self.edge_cost_sums[edge_key]) / float(count)
    ...
    def select_path(self, instance: dict[str, Any], env: FixedTreeEnvironment) -> list[str]:
        ...
        for stage_name in env.STAGE_NAMES:
            ...
            if not child_prefixes:
                raise RuntimeError(
                    "NaiveMixedAveragePolicy found no legal child prefixes. "
    ...
    def update(self, episode_result: EpisodeResult) -> None:
        ...
        for depth in range(len(leaf)):
            ...
            self.edge_cost_sums[edge_key] = self.edge_cost_sums.get(edge_key, 0.0) + observed_cost
            self.edge_cost_counts[edge_key] = self.edge_cost_counts.get(edge_key, 0) + 1

    def get_state(self) -> dict[str, Any]:
        return {
            "protocol_mode": self.protocol_mode,
            "selection_rule": "prefix_local_min_average_edge_cost",
            "update_rule": "average_selected_edges_with_observed_total_cost",
            "num_paths": len(self.paths),
            "tracked_edges": len(self.edge_cost_counts),
        }
