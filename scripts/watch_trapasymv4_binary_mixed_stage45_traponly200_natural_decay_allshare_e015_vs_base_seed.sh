...
SEED="${1:-1}"
POLL_SECONDS="${PSAGENT_TRAPASYM_V4_NATURAL_DECAY_WATCH_POLL_SECONDS:-120}"
...
LOG_FILE="${LOG_DIR}/trapasymv4_binarymixed_traponly200_natural_decay_allshare_e015_vs_base_seed${SEED}_watchdog.log"
LOCK_DIR="${LOG_DIR}/trapasymv4_binarymixed_traponly200_natural_decay_allshare_e015_vs_base_seed${SEED}_watchdog.lock"
...
runner_session_for_label() {
  local label="$1"
  printf 'psagent_trapasymv4_natdecay_%s_seed%s_runner' "$label" "$SEED"
}

wandb_session_for_label() {
  local label="$1"
  printf 'psagent_trapasymv4_natdecay_%s_seed%s_live_wandb' "$label" "$SEED"
}
...
log "v4 binary-mixed natural-decay allshare e015 vs base watchdog started; seed=${SEED} poll_seconds=${POLL_SECONDS}"
...
while true; do
  ...
  if [[ "$all_done" == "1" ]]; then
    log "all v4 binary-mixed natural-decay allshare e015 vs base runs complete; exiting watchdog"
    exit 0
  fi
  sleep "$POLL_SECONDS"
done
