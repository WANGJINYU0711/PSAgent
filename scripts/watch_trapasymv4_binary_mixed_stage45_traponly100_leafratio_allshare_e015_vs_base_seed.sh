...
SEED="${1:-1}"
POLL_SECONDS="${PSAGENT_TRAPASYM_V4_LEAFRATIO_WATCH_POLL_SECONDS:-120}"
...
LOG_FILE="${LOG_DIR}/trapasymv4_binarymixed_traponly100_leafratio_allshare_e015_vs_base_seed${SEED}_watchdog.log"
LOCK_DIR="${LOG_DIR}/trapasymv4_binarymixed_traponly100_leafratio_allshare_e015_vs_base_seed${SEED}_watchdog.lock"
ENV_FILE="scripts/trapasymv4_binarymixed_traponly100_leafratio_allshare_e015_vs_base_seed${SEED}_env.sh"
WANDB_PROJECT="psagent-llm-smoke"
WANDB_ENTITY="wangjinyu0711-microsoft"
WANDB_GROUP="trapasymv4_binarymixed_traponly100_leafratio_allshare_e015_vs_base_eta03_eps001_seed${SEED}"
GROUP_NAME="llm_v8_trapasymv4binarymixed_leafratio_allshare_e015_vs_direct_baseps_baseequalchild_traponly100_stage45_contract_promptv11b_stage45_gpt41mini_cleanv3_eta03_eps001_seed${SEED}"
...
LABELS=(
  "allshare_equalchild_leafratio_ps_e015"
...
runner_session_for_label() {
  local label="$1"
  printf 'psagent_trapasymv4_leafratio_%s_seed%s_runner' "$label" "$SEED"
}

wandb_session_for_label() {
  local label="$1"
  printf 'psagent_trapasymv4_leafratio_%s_seed%s_live_wandb' "$label" "$SEED"
}

experiment_name_for_label() {
  local label="$1"
  printf 'llm_v8_trapasymv4binarymixed_%s_traponly100_stage45_contract_promptv11b_stage45_gpt41mini_cleanv3_eta03_eps001_etashare015_seed%s' "$label" "$SEED"
}
...
while true; do
  ...
  if [[ "$all_done" == "1" ]]; then
    log "all v4 binary-mixed leaf-ratio allshare e015 vs base runs complete; exiting watchdog"
    exit 0
  fi
  sleep "$POLL_SECONDS"
done
