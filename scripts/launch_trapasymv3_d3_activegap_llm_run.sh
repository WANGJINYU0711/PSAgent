...
SEED="${1:-0}"
EXPERIMENT_NAME="llm_v8_trapasymv3efficientanchor4of5_stage45_contract_promptv11b_stage45_gpt41mini_cconfig_d3_activegap_reason55_eps001_10x10_4methods_seed${SEED}"
WANDB_GROUP="trapasymv3_d3_activegap_seed${SEED}"
RUN_DIR="tmp/${EXPERIMENT_NAME}"
METHOD_SESSION="psagent_trapasymv3_d3_activegap_seed${SEED}_4m"
LIVE_SESSION="psagent_trapasymv3_d3_activegap_seed${SEED}_live_wandb"
MONITOR_SESSION="psagent_trapasymv3_d3_activegap_seed${SEED}_monitor"
ENV_FILE="scripts/trapasymv3_d3_activegap_seed${SEED}_env.sh"
...
export PSAGENT_TELECOM_MODE_MISMATCH_DEEP_ON_FAST_COST="1.25"
...
tmux new-session -d -s "${LIVE_SESSION}" \
  "cd '$ROOT_DIR' && source '$ENV_FILE' && python scripts/live_wandb_partial_uploader.py --run-dir '${RUN_DIR}' --project psagent-llm-smoke --entity wangjinyu0711-microsoft --run-group '${WANDB_GROUP}' --run-name-prefix '${EXPERIMENT_NAME}_' --run-id-suffix analysis_d3_activegap --methods risky_ps direct_multistage_exp3 epsilon_exp3 risky_ps_old --poll-seconds 20 --finish-when-complete 2>&1 | tee '${RUN_DIR}/live_wandb_uploader_analysis_d3_activegap.log'"

tmux new-session -d -s "${MONITOR_SESSION}" \
  "cd '$ROOT_DIR' && source '$ENV_FILE' && python scripts/monitor_trapasym_d3_active_gap.py --run-dir '${RUN_DIR}' --experiment-name '${EXPERIMENT_NAME}' --ps-method risky_ps --baseline-methods direct_multistage_exp3 epsilon_exp3 --all-methods risky_ps direct_multistage_exp3 epsilon_exp3 risky_ps_old --tmux-method-session '${METHOD_SESSION}' --poll-seconds 60 2>&1 | tee '${RUN_DIR}/trap_d3_active_gap_monitor.stdout'"
