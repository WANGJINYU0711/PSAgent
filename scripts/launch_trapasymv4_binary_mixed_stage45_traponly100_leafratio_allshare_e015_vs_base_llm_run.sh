...
WATCHDOG_SESSION="psagent_trapasymv4_leafratio_allshare_e015_vs_base_seed${SEED}_watchdog"
...
for index in "${!LABELS[@]}"; do
  ...
  experiment_name="llm_v8_trapasymv4binarymixed_${label}_traponly100_stage45_contract_promptv11b_stage45_gpt41mini_cleanv3_eta03_eps001_etashare015_seed${SEED}"
  run_dir="${GROUP_DIR}/${label}"
  method_session="psagent_trapasymv4_leafratio_${label}_seed${SEED}_runner"
  live_session="psagent_trapasymv4_leafratio_${label}_seed${SEED}_live_wandb"
...
tmux new-session -d -s "${WATCHDOG_SESSION}" -n watchdog \
  "cd '$ROOT_DIR' && source '$ENV_FILE' && scripts/watch_trapasymv4_binary_mixed_stage45_traponly100_leafratio_allshare_e015_vs_base_seed.sh '${SEED}' 2>&1 | tee -a '${GROUP_DIR}/watchdog.stdout'"
