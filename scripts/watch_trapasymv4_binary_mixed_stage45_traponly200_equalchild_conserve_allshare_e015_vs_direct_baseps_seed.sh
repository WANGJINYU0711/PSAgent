...
SEED="${1:-1}"
POLL_SECONDS="${PSAGENT_TRAPASYM_V4_EQUALCHILD_CONSERVE_WATCH_POLL_SECONDS:-120}"
...
LOG_FILE="${LOG_DIR}/trapasymv4_binarymixed_traponly200_equalchild_conserve_allshare_e015_vs_direct_baseps_seed${SEED}_watchdog.log"
LOCK_DIR="${LOG_DIR}/trapasymv4_binarymixed_traponly200_equalchild_conserve_allshare_e015_vs_direct_baseps_seed${SEED}_watchdog.lock"
ENV_FILE="scripts/trapasymv4_binarymixed_traponly200_equalchild_conserve_allshare_e015_vs_direct_baseps_seed${SEED}_env.sh"
WANDB_PROJECT="psagent-llm-smoke"
WANDB_ENTITY="wangjinyu0711-microsoft"
WANDB_GROUP="trapasymv4_binarymixed_traponly200_equalchild_conserve_allshare_e015_vs_direct_baseps_eta03_eps001_seed${SEED}"
GROUP_NAME="llm_v8_trapasymv4binarymixed_equalchild_conserve_allshare_e015_vs_direct_baseps_traponly200_stage45_contract_promptv11b_stage45_gpt41mini_cleanv3_eta03_eps001_seed${SEED}"
...
LABELS=(
  "allshare_equalchild_conserve_ps_e015"
  "base_exp_e015"
  "base_ps_e015"
)
METHODS=(
...
run_dir_for_label() {
  local label="$1"
  printf '%s/%s' "$GROUP_DIR" "$label"
}

runner_session_for_label() {
  local label="$1"
  printf 'psagent_trapasymv4_equalchild_conserve_%s_seed%s_runner' "$label" "$SEED"
}

wandb_session_for_label() {
  local label="$1"
  printf 'psagent_trapasymv4_equalchild_conserve_%s_seed%s_live_wandb' "$label" "$SEED"
}

experiment_name_for_label() {
  local label="$1"
  printf 'llm_v8_trapasymv4binarymixed_%s_traponly200_stage45_contract_promptv11b_stage45_gpt41mini_cleanv3_eta03_eps001_etashare015_seed%s' "$label" "$SEED"
}
...
log "v4 binary-mixed equal-child conserve allshare e015 vs direct/baseps watchdog started; seed=${SEED} poll_seconds=${POLL_SECONDS}"
...
while true; do
  ...
  if [[ "$all_done" == "1" ]]; then
    log "all v4 binary-mixed equal-child conserve allshare e015 vs direct/baseps runs complete; exiting watchdog"
    exit 0
  fi
  sleep "$POLL_SECONDS"
done
