...
WATCHDOG_SESSION="psagent_trapasymv4_equalchild_e015_vs_direct_seed${SEED}_watchdog"
...
for index in "${!LABELS[@]}"; do
  ...
  method_session="psagent_trapasymv4_equalchild_${label}_seed${SEED}_runner"
  live_session="psagent_trapasymv4_equalchild_${label}_seed${SEED}_live_wandb"
...
tmux new-session -d -s "${WATCHDOG_SESSION}" -n watchdog \
  "cd '$ROOT_DIR' && source '$ENV_FILE' && scripts/watch_trapasymv4_binary_mixed_stage45_traponly200_equalchild_allshare_e015_vs_direct_seed.sh '${SEED}' 2>&1 | tee -a '${GROUP_DIR}/watchdog.stdout'"
