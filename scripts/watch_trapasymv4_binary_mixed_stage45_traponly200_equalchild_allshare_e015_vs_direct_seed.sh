...
SEED="${1:-1}"
POLL_SECONDS="${PSAGENT_TRAPASYM_V4_EQUALCHILD_WATCH_POLL_SECONDS:-120}"
...
LOG_FILE="${LOG_DIR}/trapasymv4_binarymixed_traponly200_equalchild_allshare_e015_vs_direct_seed${SEED}_watchdog.log"
LOCK_DIR="${LOG_DIR}/trapasymv4_binarymixed_traponly200_equalchild_allshare_e015_vs_direct_seed${SEED}_watchdog.lock"
ENV_FILE="scripts/trapasymv4_binarymixed_traponly200_equalchild_allshare_e015_vs_direct_seed${SEED}_env.sh"
WANDB_PROJECT="psagent-llm-smoke"
WANDB_ENTITY="wangjinyu0711-microsoft"
WANDB_GROUP="trapasymv4_binarymixed_traponly200_equalchild_allshare_e015_vs_direct_eta03_eps001_seed${SEED}"
GROUP_NAME="llm_v8_trapasymv4binarymixed_equalchild_allshare_e015_vs_direct_traponly200_stage45_contract_promptv11b_stage45_gpt41mini_cleanv3_eta03_eps001_seed${SEED}"
...
runner_session_for_label() {
  local label="$1"
  printf 'psagent_trapasymv4_equalchild_%s_seed%s_runner' "$label" "$SEED"
}

wandb_session_for_label() {
  local label="$1"
  printf 'psagent_trapasymv4_equalchild_%s_seed%s_live_wandb' "$label" "$SEED"
}
...
log "v4 binary-mixed equal-child allshare e015 vs direct watchdog started; seed=${SEED} poll_seconds=${POLL_SECONDS}"
...
while true; do
  ...
  if [[ "$all_done" == "1" ]]; then
    log "all v4 binary-mixed equal-child allshare e015 vs direct runs complete; exiting watchdog"
    exit 0
  fi
  sleep "$POLL_SECONDS"
done
