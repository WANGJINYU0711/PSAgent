...
tmux new-session -d -s "${LIVE_SESSION}" -n wandb \
  "cd '$ROOT_DIR' && source '$ENV_FILE' && python scripts/live_wandb_partial_uploader.py --run-dir '${RUN_DIR}' --project psagent-llm-smoke --entity wangjinyu0711-microsoft --run-group '${WANDB_GROUP}' --run-name-prefix '${EXPERIMENT_NAME}_' --run-id-suffix analysis_d2_activegapv2_nomodecost_mechanisms_chooserfixv2 --methods theta_guided_agent agent_only --poll-seconds 20 --finish-when-complete 2>&1 | tee '${RUN_DIR}/live_wandb_uploader_analysis_d2_activegapv2_nomodecost_mechanisms_chooserfixv2.log'"
