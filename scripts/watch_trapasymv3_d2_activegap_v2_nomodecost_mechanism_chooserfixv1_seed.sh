...
LOG_FILE="${LOG_DIR}/trapasymv3_d2_activegapv2_nomodecost_mechanisms_chooserfixv1_seed${SEED}_watchdog.log"
LOCK_DIR="${LOG_DIR}/trapasymv3_d2_activegapv2_nomodecost_mechanisms_chooserfixv1_seed${SEED}_watchdog.lock"

EXPERIMENT_NAME="llm_v8_trapasymv3efficientanchor4of5_stage45_contract_promptv11b_stage45_gpt41mini_cconfig_d2_activegapv2_nomodecost_reason135_eps001_10x10_mechanisms_chooserfixv1_seed${SEED}"
RUN_DIR="tmp/${EXPERIMENT_NAME}"
MECHANISM_SESSION="psagent_trapasymv3_d2_activegapv2_nomodecost_seed${SEED}_mech_chooserfixv1"
LIVE_SESSION="psagent_trapasymv3_d2_activegapv2_nomodecost_seed${SEED}_mech_chooserfixv1_wandb"
MERGE_SESSION="psagent_trapasymv3_d2_activegapv2_nomodecost_seed${SEED}_mech_chooserfixv1_merge"
LAUNCH_COMMAND="bash scripts/launch_trapasymv3_d2_activegap_v2_nomodecost_mechanism_llm_run_chooserfixv1.sh ${SEED}"
ENV_FILE="scripts/trapasymv3_d2_activegapv2_nomodecost_seed${SEED}_env.sh"
WANDB_GROUP="trapasymv3_d2_activegapv2_nomodecost_mechanisms_chooserfixv1_seed${SEED}"
MECHANISMS=(
  theta_guided_agent
  agent_only
)
...
restart_wandb() {
  ...
  log "restarting wandb uploader for mechanisms chooserfixv1"
  tmux new-session -d -s "$LIVE_SESSION" -n wandb \
    "cd '$ROOT_DIR' && source '$ENV_FILE' && python scripts/live_wandb_partial_uploader.py --run-dir '${RUN_DIR}' --project psagent-llm-smoke --entity wangjinyu0711-microsoft --run-group '${WANDB_GROUP}' --run-name-prefix '${EXPERIMENT_NAME}_' --run-id-suffix analysis_d2_activegapv2_nomodecost_mechanisms_chooserfixv1 --methods ${MECHANISMS[*]} --poll-seconds 20 --finish-when-complete 2>&1 | tee -a '${RUN_DIR}/live_wandb_uploader_analysis_d2_activegapv2_nomodecost_mechanisms_chooserfixv1.log'"
}

relaunch_run() {
  log "relaunching mechanisms chooserfixv1 from checkpoint with: ${LAUNCH_COMMAND}"
  eval "$LAUNCH_COMMAND" >>"$LOG_FILE" 2>&1
  log "relaunch finished for mechanisms chooserfixv1"
}

log "mechanism chooserfixv1 watchdog started; seed=${SEED} poll_seconds=${POLL_SECONDS}"
...
while true; do
  ...
  if mechanism_session_healthy && support_sessions_healthy; then
    log "healthy mechanisms chooserfixv1 session=${MECHANISM_SESSION} live=${LIVE_SESSION} merge=${MERGE_SESSION}"
  else
    log "unhealthy mechanisms chooserfixv1; attempting restart from checkpoint"
    relaunch_run
  fi
  ...
  if wandb_process_alive && ! wandb_state_stale; then
    log "healthy wandb mechanisms chooserfixv1"
  else
    log "unhealthy wandb mechanisms chooserfixv1"
    restart_wandb
  fi
