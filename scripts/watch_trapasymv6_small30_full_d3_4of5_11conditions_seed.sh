...
SEED="${1:-1}"
POLL_SECONDS="${PSAGENT_TRAPASYM_V6_SMALL30_FULL_D3_WATCH_POLL_SECONDS:-120}"
RUNNER_PROGRESS_STALE_SECONDS="${PSAGENT_TRAPASYM_RUNNER_PROGRESS_STALE_SECONDS:-1200}"
RUNNER_STARTUP_GRACE_SECONDS="${PSAGENT_TRAPASYM_V6_SMALL30_FULL_D3_RUNNER_STARTUP_GRACE_SECONDS:-240}"
RUNNER_LOG_STALE_SECONDS="${PSAGENT_TRAPASYM_V6_SMALL30_FULL_D3_RUNNER_LOG_STALE_SECONDS:-1200}"
...
LOG_FILE="${LOG_DIR}/trapasymv6small30_full_d3_4of5_11conditions_seed${SEED}_watchdog.log"
LOCK_DIR="${LOG_DIR}/trapasymv6small30_full_d3_4of5_11conditions_seed${SEED}_watchdog.lock"
...
runner_session_for_label() {
  printf 'psagent_trapasymv6small30_full_d3_%s_seed%s_runner' "$1" "$SEED"
}

wandb_session_for_label() {
  printf 'psagent_trapasymv6small30_full_d3_%s_seed%s_wandb' "$1" "$SEED"
}
...
startup_grace_seconds = float(
    os.environ.get(
        "PSAGENT_TRAPASYM_V6_SMALL30_FULL_D3_RUNNER_STARTUP_GRACE_SECONDS",
        str(stale_seconds),
    )
)
log_stale_seconds = float(
    os.environ.get(
        "PSAGENT_TRAPASYM_V6_SMALL30_FULL_D3_RUNNER_LOG_STALE_SECONDS",
        str(stale_seconds),
    )
)
...
log "v6 small30 full d3 4of5 11-condition watchdog started seed=${SEED} poll=${POLL_SECONDS}s"
...
while true; do
  ...
  for idx in "${!LABELS[@]}"; do
    ...
    all_done=0
    if runner_process_alive "$run_dir" "$method" "$runner_type" && ! PSAGENT_TRAPASYM_V6_SMALL30_FULL_D3_RUNNER_STARTUP_GRACE_SECONDS="$RUNNER_STARTUP_GRACE_SECONDS" PSAGENT_TRAPASYM_V6_SMALL30_FULL_D3_RUNNER_LOG_STALE_SECONDS="$RUNNER_LOG_STALE_SECONDS" runner_progress_stale "$run_dir" "$method" "$RUNNER_PROGRESS_STALE_SECONDS"; then
