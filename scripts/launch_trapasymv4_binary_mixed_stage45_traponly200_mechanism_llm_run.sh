...
tmux new-session -d -s "${MECHANISM_SESSION}" -n theta \
  "cd '$ROOT_DIR' && source '$ENV_FILE' && python scripts/run_shared_basin_mechanism_repeated_smoke.py run-mechanism --run-dir '${RUN_DIR}' --mechanism theta_guided_agent 2>&1 | tee -a '${RUN_DIR}/theta_guided_agent/tmux_runner.log'"
tmux new-window -t "${MECHANISM_SESSION}" -n agent_only \
  "cd '$ROOT_DIR' && source '$ENV_FILE' && python scripts/run_shared_basin_mechanism_repeated_smoke.py run-mechanism --run-dir '${RUN_DIR}' --mechanism agent_only 2>&1 | tee -a '${RUN_DIR}/agent_only/tmux_runner.log'"

tmux new-session -d -s "${LIVE_SESSION}" -n wandb \
  "cd '$ROOT_DIR' && source '$ENV_FILE' && python scripts/live_wandb_partial_uploader.py --run-dir '${RUN_DIR}' --project psagent-llm-smoke --entity wangjinyu0711-microsoft --run-group '${WANDB_GROUP}' --run-name-prefix '${EXPERIMENT_NAME}_' --run-id-suffix analysis_mechanisms_traponly200 --methods theta_guided_agent agent_only --poll-seconds 20 --finish-when-complete 2>&1 | tee -a '${RUN_DIR}/live_wandb_uploader_analysis_mechanisms_traponly200.log'"

tmux new-session -d -s "${MERGE_SESSION}" -n merge \
  "cd '$ROOT_DIR' && source '$ENV_FILE' && while [ ! -f '${RUN_DIR}/theta_guided_agent/summary.json' ] || [ ! -f '${RUN_DIR}/agent_only/summary.json' ]; do date; sleep 30; done; python scripts/run_shared_basin_mechanism_repeated_smoke.py merge-all --run-dir '${RUN_DIR}' 2>&1 | tee -a '${RUN_DIR}/merge_all.log'"
