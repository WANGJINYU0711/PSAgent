...
BarrierShare maintains two coupled mechanisms. The first is \emph{recursive selection}: at each internal node, the learner selects a child either according to aggregated subtree mass or according to a local exploration-exploitation rule. The second is \emph{barrier-limited update}: after observing terminal feedback at the selected leaf, the learner performs local updates on the sampled risky path and, if the leaf is shareable, propagates the induced leaf delta upward through the shareable prefix of the path.
...
BarrierShare uses three kinds of state variables.
...
For any node $u$, we define its subtree weight as the exact aggregate of shared leaf weights:
\begin{equation}
    W_t(u) \coloneqq \sum_{\ell \in \mathcal{L}(u)\cap \mathcal{L}_{\mathrm{shr}}} w_t(\ell).
    \label{eq:subtree_weight_def}
\end{equation}
For every safe node $u$, this quantity is maintained exactly and is used for selection. The key invariant is that within a safe subtree, $W_t(u)$ always equals the sum of the weights of its shared descendant leaves.

\paragraph{Local scores.}
For each directed edge $(u,v)$ with $v \in \mathsf{ch}(u)$, we maintain a local score $\theta_t(v\mid u)$. These scores are used only at risky nodes and are updated only on the sampled path.

\paragraph{Reach probability.}
...
\paragraph{Safe nodes.}
If the current node is safe, child selection is induced by aggregated subtree weights:
\begin{equation}
    p_t(v\mid u) = \frac{W_t(v)}{W_t(u)},
    \qquad v \in \mathsf{ch}(u).
    \label{eq:safe_selection}
\end{equation}
This rule is the full-sharing endpoint restricted to a shareable subtree. Because safe subtrees are closed under descendants, the same rule applies recursively to every child subtree below a safe node.
...
\begin{equation}
    ...
    \label{eq:risky_selection}
\end{equation}
Here $\epsilon_i \in (0,1)$ controls exploration at node $u$. This rule means that with probability $\epsilon_i$ the learner samples uniformly at random, and with probability $1-\epsilon_i$ it samples according to the exponential-weight distribution.
...
\eIf{$\ell_t \in \mathcal{L}_{\mathrm{shr}}$}{
    update $\phi_{t+1}(\ell_t)=\phi_t(\ell_t)+c_t$ and recompute $w_{t+1}(\ell_t)=\exp(-\eta\phi_{t+1}(\ell_t))$\;
    compute the induced leaf delta $\Delta_t(\ell_t)=w_{t+1}(\ell_t)-w_t(\ell_t)$\;
    propagate $\Delta_t(\ell_t)$ upward along ancestors until the first node with $g(u)=\mathrm{unshare}$, and stop there\;
}{
    do not generate any reusable ancestor update\;
}
\end{algorithm}
...
\paragraph{All-unshare regime.}
If no reusable sharing is available, then every non-root node is risky. In this case, BarrierShare reduces to a purely local hierarchical bandit learner that updates only the sampled risky edges along the path. This recovers the fully local multi-stage terminal-feedback regime.
