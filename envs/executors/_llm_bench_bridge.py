"""LLM + real tool bridge for Stage 2/3 bench-backed execution."""
...
def _filter_tools(env: Any, allowed_names: list[str]) -> list[Any]:
    tools = env.get_tools()
...
def main() -> None:
    ...
    tools = _filter_tools(env, allowed_tools)
    ...
    for step_idx in range(max_rounds):
        assistant = llm_generate(
            ...
            tools=tools,
            tool_choice="auto",
            call_name=f"psagent_{stage_name}_llm_bench",
            **llm_args,
        )
