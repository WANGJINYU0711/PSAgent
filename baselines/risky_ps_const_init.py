"""Constant shared-init variants for the top-performing PS-family policies.

These variants keep the original sampling and update rules intact and only
replace the initial shared edge aggregate with a structure-agnostic constant.
The goal is to remove the subtree-size prior from the safe shared suffix while
